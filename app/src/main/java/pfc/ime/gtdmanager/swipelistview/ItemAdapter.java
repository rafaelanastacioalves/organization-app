...
//		CheckLine chlnTemp = actBxInbox.getCheckLineByID(chlnID);
		if(chk.isChecked()) {
			...
			Toast.makeText(context, "Checbox de ID " + String.valueOf(data.get(position).getId()) + " marcado! Seu texto é: " + tbTxt.getText() , Toast.LENGTH_SHORT).show();
...
//			tbTxt.setTextColor(Color.BLACK);
			Toast.makeText(context, "Checbox de ID " + String.valueOf(data.get(position).getId()) + " desmarcado!Seu texto é: " + tbTxt.getText(), Toast.LENGTH_SHORT).show();
